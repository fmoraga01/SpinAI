...
1. [ ] **Import the reduced-motion hook.** In `app/components/Schedule.tsx`,
...
2. [ ] **Add the entrance keyframes.** Add a `scheduleRowIn` `@keyframes`
...
3. [ ] **Apply the animation per row, staggered.** On each row's existing
...
4. [ ] **Verify no interference with existing behavior.** Confirm the row's
...
6. [ ] **Lint/build check.** Run `npm run lint` and `npm run build`; both
