...
- [ ] 1.1 Crear `app/api/team/route.ts` con `GET` — reemplaza `loadData()`.
...
- [ ] 1.2 Crear `app/api/team/members/route.ts` con `POST` — reemplaza
...
- [ ] 1.3 Crear `app/api/team/members/[id]/route.ts` con `PATCH`
...
- [ ] 1.4 Crear `app/api/team/members/[id]/toggle/route.ts` con `POST` —
      reemplaza `toggleMember`. `isAuthenticated` + 401.
- [ ] 1.5 Crear `app/api/team/assignments/[id]/route.ts` con `DELETE` —
      reemplaza `removeAssignment` (incluye limpiar logs si quedan 0
      asignaciones). `isAuthenticated` + 401.
- [ ] 1.6 Crear `app/api/team/assignments/swap/route.ts` con `POST` —
      reemplaza `swapAssignmentMembers`, incluyendo el insert en
      `assignment_logs`. `isAuthenticated` + 401.
- [ ] 1.7 Crear `app/api/team/assignments/bulk-preview/route.ts` con `GET`
...
- [ ] 2.2 En cada función, si la respuesta no es OK, lanzar
...
- [ ] 3.1 Crear `app/api/public/news/route.ts` con `GET` (query `page`) —
      reemplaza el acceso a Supabase de `loadNews`. Sin auth.
- [ ] 3.2 Crear `app/api/public/research/route.ts` con `GET` (query
      `limit`) — reemplaza el acceso a Supabase de `loadResearchPapers`.
      Sin auth.
- [ ] 3.3 Crear `app/api/public/hf-trending/route.ts` con `GET` —
      reemplaza el acceso a Supabase de `loadHfTrending`. Sin auth.
- [ ] 3.4 Crear `app/api/public/ai-models/route.ts` con `GET` — reemplaza
      el acceso a Supabase de `loadAiModels`. Sin auth.
- [ ] 3.5 Reescribir `lib/news.ts`, `lib/research.ts`, `lib/hfTrending.ts`,
...
- [ ] 4.1 `app/api/cron/refresh-news/route.ts`: reemplazar
      `getSupabase()` por `getSupabaseAdmin()`.
- [ ] 4.2 `app/api/cron/refresh-research/route.ts`: idem.
- [ ] 4.3 `app/api/cron/refresh-hf-trending/route.ts`: idem.
- [ ] 4.4 `app/api/cron/refresh-state-of-ai/route.ts`: idem.
- [ ] 4.5 `app/api/cron/notify/route.ts`: idem.
- [ ] 4.6 `app/api/notify/route.ts`: idem.
- [ ] 4.7 Verificación manual: documentar en `progress/impl_<feature>.md`
...
- [ ] 5.2 Documentar en el propio archivo `.sql` (comentario) el hallazgo
      de auditoría y referencia a este spec, siguiendo el estilo de
      comentarios de migraciones ya existentes.
- [ ] 5.3 Dejar explícito en `progress/impl_<feature>.md` — en mayúsculas
...
- [ ] 6.1 `npm run verify` pasa (lint + build + test + check-sdd-state).
...
- [ ] 6.3 Si algún paso de QA no se puede ejercitar en este sandbox por
