...
import { AiModel, bestVariantPerSlug, buildExecutiveSummary, formatIndex, formatPrice, loadAiModels, rollingReleaseCounts } from "@/lib/stateOfAi";
...
export default function StateOfAiPage() {
  ...
  const releaseTrend = useMemo(() => rollingReleaseCounts(deduped, 6, 30, now), [deduped, now]);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--color-bg)", position: "relative" }}>
      <div style={{ position: "fixed", inset: 0, zIndex: 0 }}>
        <AnimatedGrid variant="background" intensity={0.2} />
      </div>
      ...
      <div className="max-w-5xl mx-auto px-6" style={{ paddingTop: 56, paddingBottom: 80 }}>
        ...
        <p style={{ fontSize: 15.5, color: "var(--color-text-secondary)", maxWidth: 560, lineHeight: "24px", margin: "0 0 40px" }}>
          ...
          cómo evolucionan y cuándo conviene utilizar cada uno.
        </p>
        ...
        {!loading && stats && (
          <>
            ...
            {/* Stat tiles — precio/calidad ~7% más grande, laboratorio 15% más grande */}
            <div className="grid grid-cols-1 md:grid-cols-[1fr_1.07fr_1.15fr] gap-3" style={{ marginBottom: 16 }}>
              <StatTile
                value={stats.releases30}
                label="Modelos lanzados en los últimos 30 días"
                index={0}
                trend={releaseTrend.map((d) => ({ value: d.count, label: d.label }))}
                trendLabel={`Modelos lanzados en ventanas móviles de 30 días, cada una terminando en la fecha indicada: ${releaseTrend.map((d) => `${d.label}: ${d.count}`).join(", ")}.`}
              />
              <StatTile
                value={stats.bestValue ? stripMaxSuffix(stats.bestValue.name) : "—"}
                label="Modelo con mejor relación precio/calidad"
                caption={stats.bestValue ? `${formatPrice(stats.bestValue.priceBlended1m)}/1M tokens` : undefined}
                index={1}
              />
              <StatTile
