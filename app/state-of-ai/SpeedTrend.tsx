import { AiModel, speedTrend } from "@/lib/stateOfAi";
...
export default function SpeedTrend({ models }: { models: AiModel[] }) {
  const trend = speedTrend(models);
  if (trend.length < 6) return null;

  const half = Math.floor(trend.length / 2);
  const oldAvg = trend.slice(0, half).reduce((sum, p) => sum + p.tokensPerSecond, 0) / half;
  const newAvg = trend.slice(half).reduce((sum, p) => sum + p.tokensPerSecond, 0) / (trend.length - half);
  const pctChange = ((newAvg - oldAvg) / oldAvg) * 100;
  const improving = pctChange > 0;
  const color = improving ? MARK : DOWN;

  const points = sparklinePoints(trend.map((p) => p.tokensPerSecond));
  ...
  return (
    ...
    >
      ...
      <svg
        ...
        height={H}
        role="img"
        aria-label={`Velocidad de inferencia: ${improving ? "subió" : "bajó"} ${Math.abs(pctChange).toFixed(0)}% en los últimos ${trend.length} modelos lanzados.`}
