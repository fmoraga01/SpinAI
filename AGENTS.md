...
arriba — no lo reemplaza. Ver `docs/specs.md` para el proceso completo
(incluye qué cambios NO necesitan spec, sección "Alcance") y
...
- **Antes de ejecutar cualquier promoción a `main`**, leer `docs/prod-promotion-checklist.md` completo y seguirlo paso a paso (migraciones SQL pendientes de aplicar en Supabase prod, env vars a confirmar/crear, hallazgos de seguridad pendientes de resolver antes del cutover). [2026-07-30] Directriz explícita del usuario: no asumir que el merge es trivial solo porque `npm run verify` está en verde — ese archivo se creó porque `dev` puede tener migraciones/features que nunca se aplicaron en prod.
